...
def create_2d_scatter_plot(input_filename, keys, title, xaxis, yaxis):
    ...
    x_bin_edges = np.linspace(50, 550, 20)  # 18 bins from 50 to 500
    y_bin_edges = np.linspace(min(y_values), max(y_values), 20)
...
#    popt, pcov = curve_fit(linear_fit, x_centers, y_avg)
    popt, pcov = curve_fit(linear_fit, x_centers, y_avg, sigma = y_err)

    print(popt)
    chi_sq = chi_squared(y_avg, linear_fit(x_centers, *popt), y_err)
    print(f"Chi-squared for {pair[0]} vs {pair[1]}: {chi_sq}")
    print(pcov)

    # Plot the 2D scatter plot with average Y values
    plt.scatter(x_values, y_values, alpha=0.3, label=f'{pair[0]} vs {pair[1]}')

    # Plot the means, error bars, and line fit
    plt.plot(0.5 * (xedges[:-1] + xedges[1:]), y_avg, marker='o', linestyle='-')
    plt.errorbar(x_centers, y_avg, yerr=y_err, fmt='o', label=f'{pair[0]} vs {pair[1]}')
...
if __name__ == "__main__":
    ...
    create_2d_scatter_plot(input_filename, keys, title, xaxis, yaxis)
