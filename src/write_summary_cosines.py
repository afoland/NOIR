...
def calc_M(lratio,similarity):
    if (similarity <= 0):
        return 0
    else:
        return (np.log(lratio) / np.log(similarity))
...
def process_jsonl(input_filename, output_filename):
    with jsonlines.open(input_filename, 'r') as reader:
        with jsonlines.open(output_filename, 'w') as writer:
            for line in reader:
                # Embed strings
                text_vector = embed_string(line.get('text', ''))
                summary1_vector = embed_string(line.get('summary_1', ''))
                summary2_vector = embed_string(line.get('summary_2', ''))
                summary3_vector = embed_string(line.get('summary_3', ''))
                ...
                line['M_01'] = calc_M(lratio_01,similarity_01)
                line['M_02'] = calc_M(lratio_02,similarity_02)
                line['M_03'] = calc_M(lratio_03,similarity_03)
                line['M_12'] = calc_M(lratio_12,similarity_12)
                line['M_13'] = calc_M(lratio_13,similarity_13)
                line['M_23'] = calc_M(lratio_23,similarity_23)
